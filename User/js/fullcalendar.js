		...
		function render(inc) {
			if (_element.offsetWidth !== 0) { // visible on the screen
				...
				if (header) {
					...
					header.find('h3.fc-header-title').html(view.title);
			...
			if (buttonStr) {
				...
				$.each(buttonStr.split(' '), function(i) {
					...
					$.each(this.split(','), function(j, buttonName) {
						if (buttonName == 'title') {
							tr.append("<td><h3 class='fc-header-title'/></td>");
							if (prevButton) {
				...
				for (k=0; k<segs.length; k++) {
					...
					eventElement = $("<div class='" + className + event.className.join(' ') + "'/>")
						.append(eventAnchor = $("<a/>")
							...
							.append($("<span class='fc-event-title' style='color:white;'/>")
								.text(event.title)));
					if (event.url) {
						...
						.append(anchorElement = $("<a><span class='fc-event-bg'/></a>")
							.append(timeElement = $("<span class='fc-event-time'/>")
								.text(formatDates(event.start, event.end, view.option('timeFormat'))))
                       			.append(titleElement = $("<span class='fc-event-title' style=\"color:white;\"/>")
								.text(event.title)))
					if (event.url) {
						anchorElement.attr('href', event.url);
					}
